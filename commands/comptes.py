...
def create_connection(db_path=db_path):
    ...
    try:
        ...
        print("Connection to SQLite DB for comptes.py successful")
    except Error as e:
...
def update_montant(cursor, source, destination, ajout, compte_name, compte_owner):
    ...
    montant = cursor.execute(
        f"SELECT SUM(montant) FROM comptes WHERE source = '{source}' AND destination = '{destination}' AND compte_name = '{compte_name}'"
    ).fetchall()
    print(montant)
    ...
    if montant != None:
        ...
        print(montant)
        cursor.execute(
            f"UPDATE comptes SET montant = {montant} WHERE source = '{source}' AND destination = '{destination}' AND compte_name = '{compte_name}'"
        )
    else:
        cursor.execute(
            f"INSERT INTO comptes (source, destination, montant, compte_name, compte_owner) VALUES  ('{source}', '{destination}', {ajout}, '{compte_name}', '{compte_owner}')"
        )
...
def create_sankey(compte_name, db_path=db_path):
    ...
    print("Création du sankey ...")
    print(fig)
    fig.write_image(rf"csv_files\comptes\{compte_name}.png", format="png")
...
class AddIncomeModal(discord.ui.Modal, title="Revenu"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        add_source(
            ...
            destination=destination,
            amount=amount,
            compte_name=self.compte_name,
            compte_owner=user,
        )
        img = discord.File(
            rf"csv_files\comptes\{self.compte_name}.png", f"{self.compte_name}.png"
        )
        embed = discord.Embed(
            color=discord.Colour.random(), title=self.compte_name
        ).set_image(url=f"attachment://{f'{self.compte_name}.png'}")
        await interaction.followup.edit_message(
                message_id=interaction.message.id,
            embed=embed, attachments=[img], view=AccountButtons(user, self.compte_name)
        )
...
class AccountButtons(discord.ui.View):
    ...
    @discord.ui.button(
        label="Ajouter un revenu / une dépense", style=discord.ButtonStyle.blurple
    )
...
class UserSelect(discord.ui.Select):
    ...
    def __init__(self, created_account):
        options = [
            discord.SelectOption(label=compte_name) for compte_name in created_account
        ]

        super().__init__(
            placeholder=r"Choisis ton compte",
            min_values=1,
            max_values=1,
            options=options,
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        path = rf"csv_files\comptes\{compte}.png"
        create_sankey(compte)
        if os.path.exists(path):
            img = discord.File(rf"csv_files\comptes\{compte}.png", f"{compte}.png")
            embed = discord.Embed(
                color=discord.Colour.random(), title=compte
            ).set_image(url=f"attachment://{f'{compte}.png'}")
        else:
        ...
        conn = create_connection()
        cursor = conn.cursor()
        compte_owner = cursor.execute(
            f"SELECT compte_owner FROM comptes WHERE compte_name == '{compte}' LIMIT 1"
        ).fetchone()[0]
        conn.close()
        if compte_owner == user:
            await interaction.followup.edit_message(
                message_id=interaction.message.id,
                embed=embed, attachments=[img], view=AccountButtons(user, compte)
            )
        else:
            await interaction.followup.edit_message(
                message_id=interaction.message.id,
                embed=embed, attachments=[img], view=MISSING
            )
...
class CreateDiagram(discord.ui.Modal, title="Créer un diagramme"):  #
    ...
    async def on_submit(self, interaction: discord.Interaction):
        user = interaction.user.name
        compte_name = self.compte_name.value.replace(" ", "_")
        conn = create_connection()
        cursor = conn.cursor()
        cursor.execute(
            f"INSERT INTO comptes (source, destination, montant, compte_name, compte_owner) VALUES ('init', 'Budget', 0, '{compte_name}', '{user}')"
        )
        conn.commit()
        conn.close()
        create_sankey(compte_name)
        img = discord.File(
            rf"csv_files\comptes\{compte_name}.png", f"{compte_name}.png"
        )
        embed = discord.Embed(
            color=discord.Colour.random(), title=compte_name
        ).set_image(url=f"attachment://{f'{compte_name}.png'}")
        await interaction.response.edit_message(
            embed=embed, attachments=[img], view=AccountButtons(user, compte_name)
        )
...
class SelectActionButtons(discord.ui.View):
    ...
    async def see_diagram(
        ...
        conn = create_connection()
        cursor = conn.cursor()
        created_account = cursor.execute(
            "SELECT DISTINCT compte_name FROM comptes"
        ).fetchall()
        conn.close()
        created_account = [el[0] for el in created_account]

        await interaction.response.edit_message(view=UserSelectView(created_account))
...
@bot.tree.command(
        guild = None,
    description="Permet de visualiser son budget",
)
