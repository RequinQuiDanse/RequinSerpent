...
class Musique:
    ...
    async def boucle_musique(self, ctx, musique):  # Func cerveau
        ...
        await bot.change_presence(
            activity=discord.Activity(type=discord.ActivityType.listening, name="Quoi?")
        )

        if (
            self.replay
        ):  # si on joue une playlist => ça trouve une autre musique et ça rejoue
    ...
    async def getUrl(
        self,
    ):  # Permet de jouer une playlist: ça récupère une musique aléatoire à partir de la toute première playlist demandée (self.choix_musique)
        """
        return a youtube url or a music name, depends on what is asked
        also manage if a playlist is asked or no
        """
        if (
            self.asked_playlist == "Aléatoire"
        ):  # Si aléatoire était la demande, alors on prend une playlist aléatoire
            self.asked_playlist = (
                str(random.choice(glob.glob("playlist/*.csv")))
                .replace("\\", "/")
                .replace("playlist/", "")
                .replace(".csv", "")
            )
    ...
    async def ensure_voice(
        self,
    ):  # Etape obligatoire permettant de ne pas créer de conflit
        """
        do all the stuff to encuse the bot's voice is well connected
        """
        if (
            self.ctx.voice_client is None
        ):  # si le bot n'est pas connecté, ça le connecte
            print("self.ctx.voice_client is none")
            if (
                self.ctx.author.voice
            ):  # vérifie que le demandeur de musique est bien dans un salon vocal
        ...
        ):  # Si le bot jouait de la musique au moment de l'appel, ça l'arrête
...
async def playlist(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(
        "Choisis",
        view=playlistSelectView(ctx),
        ephemeral=bool(interaction.channel_id != 942818979318210631),
    )
...
class playlistSelectView(discord.ui.View):
        ...
        for playlist in glob.glob("playlist/*.csv"):
            try:
                ...
                self.add_item(
                    discord.ui.Button(
                        style=discord.ButtonStyle.blurple,
                        label=str(playlist).replace("spotify", ""),
                        custom_id=str(playlist),
                    )
                )
            except:
    ...
    async def interaction_check(self, interaction=discord.Interaction):
        await music_class.boucle_musique(
            ctx=self.ctx, musique=interaction.data.get("custom_id")
        )
